...
llist_t *mergeList(llist_t *a, llist_t *b){
    ...
    node_t *cur = NULL;
    while (a->size && b->size) {
        llist_t *smallList = (llist_t *)((intptr_t)a*(a->head->data <= b->head->data) + (intptr_t)b*(a->head->data > b->head->data));
        if(cur){
    ...
    llist_t *remainList = (llist_t *)((intptr_t)a*(a->size > 0) + (intptr_t)b*(b->size > 0));
